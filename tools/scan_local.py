...
SECRET = re.compile(r"(?i)(token|password|secret|api[_-]?key)\s*[:=]\s*\S+|\b(?:sk|gh[op]|xoxb)[-_][\w-]{8,}")
        ...
        if path.exists():
            for line in path.read_text(errors="replace").splitlines():
                try:
                    ...
                    if item.get("schema")=="codex-ops-event/v1": ops.append({k:item.get(k) for k in ("event","timestamp","project","artifact")})
                except json.JSONDecodeError: continue
    grouped={}
    for t in threads:
        project=Path(t["cwd"]).name or "Codex"
        grouped.setdefault(project,[]).append(t)
    projects=[]
    for name,items in sorted(grouped.items(),key=lambda x:x[1][0]["at"],reverse=True)[:24]:
        projects.append({"id":slug(name),"name":name.replace("-"," ").title(),"area":"Local work","status":"started","summary":f"{len(items)} recent Codex thread(s); status needs evidence review.","next":"Review the newest thread and repository state","updatedAt":items[0]["at"],"score":{"impact":3,"urgency":2,"unlock":2,"staleness":1,"risk":1,"effort":2},"milestones":[],"evidence":[{"id":hashlib.sha256(items[0]["id"].encode()).hexdigest()[:12],"type":"codex","label":items[0]["label"],"at":items[0]["at"],"confidence":.6}]})
